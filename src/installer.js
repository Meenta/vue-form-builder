...
const VueFormBuilderInstaller = function(Vue, properties = {}) {
  ...
  if (
    !properties.hasOwnProperty("globalInjection") ||
    properties.globalInjection
  ) {
...
const extendingControls = function(moreControlObject) {
  ...
  for (let iKey = 0; iKey < allKeys.length; iKey++) {
    ...
    if (CONTROLS.hasOwnProperty(key)) {
      throw new TypeError(
        `Extend-Control-Error: Your '${key}' control is duplicated with our build-in Controls. Please change to another key name instead.`
      );
    }
  }
...
const extendingValidations = function(validationObj) {
  ...
  for (let iKey = 0; iKey < allKeys.length; iKey++) {
    ...
    if (VALIDATION_RULES.hasOwnProperty(key)) {
      throw new TypeError(
        `Extend-Validation-Error: Your '${key}' validation is duplicated with our build-in Validation. Please change to another key name instead.`
      );
    }
  }
